...
async def main():
    ...
    # 执行创建文件操作
    result = await sb_files_tool.execute(
        action="create_file", file_path="src/a.txt", file_contents="aaaaa1111"
    )
    print(result)
    # 执行字符串替换操作
    ...
    # 执行全文件重写操作
    result = await sb_files_tool.execute(
        action="full_file_rewrite", file_path="src/a.txt", file_contents="1234567"
    )
    print(result)
    # 执行删除文件操作
